...
public class SalesFactsController : ControllerBase
{
    ...
    // GET: api/SalesFacts/paged?pageSize=10&pageNumber=1
    [HttpGet("paged")]
    public async Task<ActionResult<PagedResponse<VwSalesFact>>> GetSalesFactsPaged([FromQuery] int pageSize, [FromQuery] int pageNumber = 1)
    {
        if (pageSize <= 0)
        {
        ...
        var totalRecords = await _context.VwSalesFacts.CountAsync();
        var totalPages = (int)Math.Ceiling(totalRecords / (double)pageSize);

        var items = await _context.VwSalesFacts
            .OrderBy(sf => sf.OrderId)
            .ThenBy(sf => sf.ProductName)
            .Skip((pageNumber - 1) * pageSize)
            .Take(pageSize)
            .ToListAsync();

        return Ok(new PagedResponse<VwSalesFact>
        {
            Items = items,
